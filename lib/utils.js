...
// Max length for custom field display before truncation
const MAX_FIELD_LENGTH = 500;

// Format custom field value for display
function formatCustomFieldValue(value, truncate = true) {
  ...
  // Object with common patterns
    ...
    if (value.displayName) result = value.displayName;
    else if (value.name) result = value.name;
    else if (value.value) result = value.value;
    else if (value.content) result = JSON.stringify(value.content);
    else result = null;
  }

  // Truncate long strings
  if (truncate && result && result.length > MAX_FIELD_LENGTH) {
    return { truncated: true, length: result.length };
  }
...
function hasValue(value) {
  ...
  if (typeof value === 'object') {
    return value.displayName || value.name || value.value || value.content;
  }
  return true;
}

// Display issue details
function displayIssueDetails(issue, fieldNameMap = {}) {
  ...
  // Display custom fields with values
  const customFields = Object.keys(issue.fields)
    .filter(key => key.startsWith('customfield_'))
    .filter(key => hasValue(issue.fields[key]));

  if (customFields.length > 0) {
    console.log(`\n${chalk.bold(chalk.cyan('Custom Fields:'))}`);
    ...
    for (const fieldKey of customFields) {
      ...
      const displayLabel = fieldNameMap[fieldKey]
        ? `${fieldName} (${fieldKey})`
        : fieldKey;

      if (formattedValue) {
        if (typeof formattedValue === 'object' && formattedValue.truncated) {
          console.log(`${chalk.bold(displayLabel + ':')} ${chalk.yellow(`[content too long: ${formattedValue.length} chars, use "jira issue field ${issue.key} ${fieldKey}" to view]`)}`);
        } else {
          console.log(`${chalk.bold(displayLabel + ':')} ${formattedValue}`);
        }
      }
    }
  }
